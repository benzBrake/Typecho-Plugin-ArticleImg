 ...
 * 文章顶部图片插件
 *
 * @package ArticleImg mod by <a href="http://doufu.ru/">Ryan</a>
 * @author BangZ
 * @version 1.0.3
 * @link http://bangz.me
 */
class ArticleImg_Plugin implements Typecho_Plugin_Interface
{
    ...
    public static function deactivate()
    {
        ...
        if ($delFields) {
            $db = Typecho_Db::get();
            $prefix = $db->getPrefix();
            $type = array_pop(explode('_', $db->getAdapterName()));
            if ($type == "SQLite") {
                // 2020.03.22 Fix SQLite Database Drop Error
                $rs = $db->fetchAll($db->query('PRAGMA TABLE_INFO('. $prefix  .'contents)'));
                $cols_arr = array();
                foreach($rs as $r) {
                    if ($r['name'] != 'thumb')
                        array_push($cols_arr, '`' . $r['name'] . '`');
                }
                $cols = implode(",", $cols_arr);
                $sql1 = 'CREATE TABLE `' . $prefix . 'contents_temp` AS SELECT ' . $cols . ' FROM `' . $prefix .'contents` WHERE 1 = 1;';
                $sql2 = 'DROP TABLE `' . $prefix .'contents`;';
                $sql3 = 'ALTER TABLE `' . $prefix . 'contents_temp` RENAME TO `' . $prefix .'contents`;';
                $db->query($sql1);
                $db->query($sql2);
                $db->query($sql3);
            } else {
    ...
    public static function render($value, $archive)
    {
        ...
        if ($value['thumb'] === "unknown") {
            ...
            if (empty($value['thumb'])) {
                $value['thumb'] = Typecho_Widget::widget('Widget_Options')->siteUrl . str_replace("/usr/", "usr/", __TYPECHO_PLUGIN_DIR__) . '/ArticleImg/rand/' . mt_rand(0, 9) . '.jpg';
            }
